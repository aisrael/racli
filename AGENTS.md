...
- When searching for symbols, DO NOT USE `grep`. Try the MCP search tool first. Fall back to `racli search` CLI. Only fall back to `grep` when `racli search` doesn't return anything meaningful.
